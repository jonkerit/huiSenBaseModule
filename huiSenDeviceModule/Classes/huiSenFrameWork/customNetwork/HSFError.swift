...
/// `HSFError` is the error type returned by Alamofire. It encompasses a few different types of errors, each with
/// their own associated reasons.
public enum HSFError: Error {
    ...
    /// embedded within an `HSFError.multipartEncodingFailed` `.inputStreamReadFailed` case.
...
extension Error {
    /// Returns the instance cast as an `HSFError`.
    public var asHSFError: HSFError? {
        self as? HSFError
    }

    /// Returns the instance cast as an `HSFError`. If casting fails, a `fatalError` with the specified `message` is thrown.
    public func asHSFError(orFailWith message: @autoclosure () -> String, file: StaticString = #file, line: UInt = #line) -> HSFError {
        guard let HSFError = self as? HSFError else {
            fatalError(message(), file: file, line: line)
        }
        return HSFError
    }

    /// Casts the instance as `HSFError` or returns `defaultHSFError`
    func asHSFError(or defaultHSFError: @autoclosure () -> HSFError) -> HSFError {
        self as? HSFError ?? defaultHSFError()
    }
}
...
extension HSFError.ParameterEncodingFailureReason {
...
extension HSFError.ParameterEncoderFailureReason {
    var underlyingError: Error? {
        switch self {
...
extension HSFError.MultipartEncodingFailureReason {
    var url: URL? {
        switch self {
...
extension HSFError.ResponseValidationFailureReason {
...
extension HSFError.ResponseSerializationFailureReason {
...
extension HSFError.ServerTrustFailureReason {
    var output: HSFError.ServerTrustFailureReason.Output? {
        switch self {
...
extension HSFError: LocalizedError {
...
extension HSFError.URLRequestValidationFailureReason {
    var localizedDescription: String {
        switch self {

...
#error JKProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "JKProgressHUD.h"
#import "JKIndefiniteAnimatedView.h"
#import "JKProgressAnimatedView.h"
#import "JKRadialGradientLayer.h"

#if huiSenFrameWork
#import <huiSenFrameWork/huiSenFrameWork-Swift.h>
#else
#import "huiSenSmart-Swift.h"
#endif

NSString * const JKProgressHUDDidReceiveTouchEventNotification = @"JKProgressHUDDidReceiveTouchEventNotification";
NSString * const JKProgressHUDDidTouchDownInsideNotification = @"JKProgressHUDDidTouchDownInsideNotification";
NSString * const JKProgressHUDWillDisappearNotification = @"JKProgressHUDWillDisappearNotification";
NSString * const JKProgressHUDDidDisappearNotification = @"JKProgressHUDDidDisappearNotification";
NSString * const JKProgressHUDWillAppearNotification = @"JKProgressHUDWillAppearNotification";
NSString * const JKProgressHUDDidAppearNotification = @"JKProgressHUDDidAppearNotification";

NSString * const JKProgressHUDStatusUserInfoKey = @"JKProgressHUDStatusUserInfoKey";

static const CGFloat JKProgressHUDParallaxDepthPoints = 10.0f;
static const CGFloat JKProgressHUDUndefinedProgress = -1;
static const CGFloat JKProgressHUDDefaultAnimationDuration = 0.15f;
static const CGFloat JKProgressHUDVerticalSpacing = 12.0f;
static const CGFloat JKProgressHUDHorizontalSpacing = 12.0f;
static const CGFloat JKProgressHUDLabelSpacing = 8.0f;
...
@property (nonatomic, strong) JKRadialGradientLayer *backgroundRadialGradientLayer;
@property (nonatomic, strong) UIVisualEffectView *hudView;
@property (nonatomic, strong) UILabel *statusLabel;
@property (nonatomic, strong) UIImageView *imageView;

@property (nonatomic, strong) UIView *indefiniteAnimatedView;
@property (nonatomic, strong) JKProgressAnimatedView *ringView;
@property (nonatomic, strong) JKProgressAnimatedView *backgroundRingView;
...
+ (void)setDefaultStyle:(JKProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(JKProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(JKProgressHUDAnimationType)type {
...
+ (void)setForegroundColor:(UIColor*)color {
    ...
    [self setDefaultStyle:JKProgressHUDStyleCustom];
}
...
+ (void)showWithMaskType:(JKProgressHUDMaskType)maskType {
    JKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self showProgress:JKProgressHUDUndefinedProgress status:status];
}

+ (void)showWithStatus:(NSString*)status maskType:(JKProgressHUDMaskType)maskType {
    JKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(JKProgressHUDMaskType)maskType {
    JKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(JKProgressHUDMaskType)maskType {
    JKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(JKProgressHUDMaskType)maskType {
    JKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWitJKtatus:(NSString*)status {
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(JKProgressHUDMaskType)maskType {
    JKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
    [self showSuccessWitJKtatus:status];
    [self setDefaultMaskType:existingMaskType];
...
+ (void)showErrorWitJKtatus:(NSString*)status {
...
+ (void)showErrorWithStatus:(nullable NSString*)status maskType:(JKProgressHUDMaskType)maskType {
    JKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
    [self showErrorWitJKtatus:status];
    [self setDefaultMaskType:existingMaskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(JKProgressHUDMaskType)maskType {
    JKProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImageAnimation:(nullable NSArray <UIImage *>*)imageArray status:(nullable NSString*)status {
    if (imageArray == nil || [imageArray count]>0) {
        [[self sharedView] showImages:imageArray status:status duration:1000000];
    } else {
        [[self sharedView] showProgress:JKProgressHUDUndefinedProgress status:status];
    }
}

- (void)showImages:(NSArray <UIImage *>*)imageArray status:(NSString*)status duration:(NSTimeInterval)duration {
    self.imageArray = [[NSMutableArray alloc] initWithArray:imageArray];
    __weak JKProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong JKProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
            // Stop timer
            ...
            strongSelf.progress = JKProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
            
            // Update imageView
            UIImage *image = strongSelf.imageArray.firstObject;
...
+ (void)dismissWithCompletion:(JKProgressHUDDismissCompletion)completion {
...
+ (void)dismissWithDelay:(NSTimeInterval)delay completion:(JKProgressHUDDismissCompletion)completion {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = JKProgressHUDMaskTypeNone;
        _defaultStyle = JKProgressHUDStyleLight;
        _defaultAnimationType = JKProgressHUDAnimationTypeFlat;
...
//        NSBundle *bundle = [NSBundle bundleForClass:[JKProgressHUD class]];
//        NSURL *url = [bundle URLForResource:@"JKProgressHUD" withExtension:@"bundle"];
...
//        NSBundle *bundle = [NSBundle bundleForClass:[JKProgressHUD class]];
//        NSLog(@"JKProgressHUD bundle === %@", bundle);
//        NSURL *url = [bundle URLForResource:@"JKProgressHUD" withExtension:@"bundle"];
//        NSLog(@"JKProgressHUD url === %@", url);
//        NSBundle *imageBundle = [NSBundle bundleWithURL:url];
//        NSLog(@"imageBundle === %@", imageBundle);
        _infoImage = [HSFrameworkImage imageNamed:@"info" bundleName:nil];
        _successImage = [HSFrameworkImage imageNamed:@"success" bundleName:nil];
        _errorImage = [HSFrameworkImage imageNamed:@"error" bundleName:nil];
        ...
        _fadeInAnimationDuration = JKProgressHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = JKProgressHUDDefaultAnimationDuration;
        ...
        self.accessibilityIdentifier = @"JKProgressHUD";
...
- (void)updateHUDFrame {
    ...
    hudWidth = JKProgressHUDHorizontalSpacing + MAX(labelWidth, contentWidth) + JKProgressHUDHorizontalSpacing;
    
    // |-spacing-content-(labelSpacing-label-)spacing-|
    hudHeight = JKProgressHUDVerticalSpacing + labelHeight + contentHeight + JKProgressHUDVerticalSpacing;
    if(self.statusLabel.text && (imageUsed || progressUsed)){
        // Add spacing if both content and label are used
        hudHeight += JKProgressHUDLabelSpacing;
    }
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(JKProgressHUDVerticalSpacing, (self.minimumSize.height - contentHeight - JKProgressHUDLabelSpacing - labelHeight) / 2.0f);
    ...
    if(self.progress != JKProgressHUDUndefinedProgress) {
    ...
    // Label
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + JKProgressHUDLabelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    ...
    effectX.minimumRelativeValue = @(-JKProgressHUDParallaxDepthPoints);
    effectX.maximumRelativeValue = @(JKProgressHUDParallaxDepthPoints);
    
    UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
    effectY.minimumRelativeValue = @(-JKProgressHUDParallaxDepthPoints);
    effectY.maximumRelativeValue = @(JKProgressHUDParallaxDepthPoints);
...
#else
            // If JKProgressHUD is used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo {
    return (self.statusLabel.text ? @{JKProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
#if !defined(JK_APP_EXTENSIONS) && TARGET_OS_IOS
    self.frame = [[[UIApplication sharedApplication] delegate] window].bounds;
    UIInterfaceOrientation orientation = UIApplication.sharedApplication.statusBarOrientation;
#elif !defined(JK_APP_EXTENSIONS) && !TARGET_OS_IOS
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:JKProgressHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:JKProgressHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)fadeIn:(id)data {
    ...
    if(self.defaultMaskType != JKProgressHUDMaskTypeNone) {
    ...
    if(self.backgroundView.alpha != 1.0f) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:JKProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(self.backgroundView.alpha == 1.0f){
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:JKProgressHUDDidAppearNotification
                                                                    object:self
                                                                  userInfo:[self notificationUserInfo]];
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(JKProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:JKProgressHUDWillDisappearNotification
                                                                object:nil
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(self.backgroundView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:JKProgressHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == JKProgressHUDAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[JKIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[JKIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        JKIndefiniteAnimatedView *indefiniteAnimatedView = (JKIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (JKProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[JKProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (JKProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[JKProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == JKProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == JKProgressHUDStyleDark) {
...
-(UIView *)backgroundView {
    ...
    // Update styling
    if(self.defaultMaskType == JKProgressHUDMaskTypeGradient){
        if(!_backgroundRadialGradientLayer){
            _backgroundRadialGradientLayer = [JKRadialGradientLayer layer];
        }
        ...
        if(self.defaultMaskType == JKProgressHUDMaskTypeBlack){
            _backgroundView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4];
        } else if(self.defaultMaskType == JKProgressHUDMaskTypeCustom){
...
- (UIWindow *)frontWindow {
    ...
    for (UIWindow *window in frontToBackWindows) {
        ...
        BOOL windowIJKisible = !window.hidden && window.alpha > 0;
        BOOL windowLevelSupported = (window.windowLevel >= UIWindowLevelNormal && window.windowLevel <= self.maxSupportedWindowLevel);
        BOOL windowKeyWindow = window.isKeyWindow;
			
        if(windowOnMainScreen && windowIJKisible && windowLevelSupported && windowKeyWindow) {
            return window;
        }
    }
#endif
...
- (void)fadeInEffects {
    if(self.defaultStyle != JKProgressHUDStyleCustom) {
        // Add blur effect
        UIBlurEffectStyle blurEffectStyle = self.defaultStyle == JKProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleLight;
...
- (void)setDefaultAnimationType:(JKProgressHUDAnimationType)animationType {
...
- (NSArray<UIImage *> *)imageArray {
    if (_imageArray == nil) {
        _imageArray = [[NSArray<UIImage *> alloc] init];
    }
    return _imageArray;
}
@end
